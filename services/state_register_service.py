...
from schemas.history_register_schema import CSVHistoryResponse
...
class StateRegisterService(CommonResource):
    ...
    async def get_state_csv(
        ...
        try:
            ...
            state_registers = await super().get_list(
                model=StateRegister,
                conditions=[
                    (StateRegister.date_created >= date_from_stmp),
                    (StateRegister.date_created <= date_to_stmp),
                ],
            )
            if state_registers:
                ...
                writer.writeheader()
                writer.writerows(
                    CSVHistoryResponse.from_orm(state).dict()
