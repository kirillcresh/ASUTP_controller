...
logger = get_custom_logger(
    ...
    handlers=[get_rotating_file_handler(settings.PATH_LOG_DIR, "maintenance_service.log")],
)
...
class MaintenanceService(BaseService):
    ...
    async def create_maintenance(self, data_dct: MaintenanceBodySchema, access_token_data: TokenData):
        user = (await self.session.execute(
            select(User).where(User.id == data_dct.created_by))).scalar_one_or_none()
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Пользователь с id = {data_dct.created_by} не найден.")

        action = (await self.session.execute(
                select(Action).where(Action.id == data_dct.action_id)
        )).scalar_one_or_none()
        if not action:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Действие с id = {data_dct.action_id} не найдено")
        maintenance = (
            await self.session.execute(
                insert(MaintenanceJournal).values(**data_dct.dict()).returning(MaintenanceJournal)
            )
        ).scalar_one_or_none()
